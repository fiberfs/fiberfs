...
typedef void (fbr_test_finish_f)(struct chttp_test_context*);
...
int chttp_test_readline(struct fbr_test *test, size_t append_len);
void chttp_test_parse_cmd(struct fbr_test *test);

struct fbr_test *chttp_test_convert(struct chttp_test_context *ctx);
...
size_t chttp_test_line_pos(struct fbr_test *test);
