...
void
fbr_md5_init(struct fbr_md5_ctx *md5)
{
	assert(md5);

	fbr_zero(md5);
	md5->magic = FBR_MD5_MAGIC;
...
void
fbr_md5_update(struct fbr_md5_ctx *md5, const void *input, size_t len)
{
	fbr_md5_ok(md5);
	assert_zero(md5->ready);
	assert(input || !len);

	int mdi = (int)((md5->i[0] >> 3) & 0x3F);
	...
	while (len--) {
		md5->in[mdi++] = *(uint8_t*)input;
		input = (uint8_t*)input + 1;

		if (mdi == 0x40) {
			uint32_t in[FBR_MD5_DIGEST_SIZE];
			for (size_t i = 0, ii = 0; i < FBR_MD5_DIGEST_SIZE; i++, ii += 4) {
...
void
fbr_md5_final(struct fbr_md5_ctx *md5)
{
	fbr_md5_ok(md5);
	assert_zero(md5->ready);
	...
	fbr_md5_update(md5, _MD5_PADDING, pad_len);
