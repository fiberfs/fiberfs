...
void
fbr_cmd_fuse_test_mount(struct fbr_test_context *ctx, struct fbr_test_cmd *cmd)
{
	...
	int ret = fbr_fuse_test_mount(ctx, mount, NULL, NULL);
	...
	fbr_test_ERROR(strcmp(mount, fuse_ctx->path),
		"ctx->path error: %s", fuse_ctx->path);

	fbr_test_log(ctx, FBR_LOG_VERBOSE, "Fuse mounted: %s", mount);
}

void
fbr_cmd_fuse_test_unmount(struct fbr_test_context *ctx, struct fbr_test_cmd *cmd)
{
	fbr_test_context_ok(ctx);
