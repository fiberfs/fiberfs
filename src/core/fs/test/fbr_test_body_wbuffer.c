...
_test_fetch_thread(void *arg)
{
	...
	fbr_chunk_update(fs, file, chunk, FBR_CHUNK_LOADING);
	...
	fbr_chunk_update(fs, file, chunk, FBR_CHUNK_READY);
...
static int
_test_flush_directory(struct fbr_fs *fs, struct fbr_flush_data *flush_data)
{
	fbr_fs_ok(fs);
	fbr_flush_data_ok(flush_data);
	fbr_wbuffer_ok(flush_data->wbuffers);
	assert(flush_data->flags == FBR_FLUSH_WBUFFER);

	struct fbr_wbuffer *wbuffer = flush_data->wbuffers;
	size_t errors = 0;
...
_test_fio_read_thread(void *arg)
{
	...
	while (!_TEST_OVER && count < 3) {
		...
		fbr_ASSERT(vector, "NULL vector offset: %zu length: %zu file_size: %zu",
...
_test_concurrent_fio(void)
{
	...
	if (_SHARED_FIO) {
		...
		if (_STORE_ERROR_THREAD_ID >= 0) {
			assert(ret);
			struct fbr_flush_data flush_data;
			fbr_flush_data_init(&flush_data, file, NULL, fio->wbuffers, NULL,
				FBR_FLUSH_WBUFFER, NULL);
			_test_flush_directory(fs, &flush_data);
			fbr_wbuffers_reset_lock(fs, fio);
		} else {
			assert_zero(ret);
