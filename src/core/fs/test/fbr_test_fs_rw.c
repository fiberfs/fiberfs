...
static void
_test_fs_rw_mkdir(struct fbr_request *request, fuse_ino_t parent, const char *name, mode_t mode)
{
	struct fbr_fs *fs = fbr_request_fs(request);
	assert_dev(fs->store);

	fbr_test_logs("MKDIR req: %lu parent: %lu name: %s mode: %u", request->id, parent, name,
		mode);
	...
	fbr_test_logs("** MKDIR found parent_file: '%s' (inode: %lu)",
	...
	fbr_test_logs("** MKDIR found directory inode: %lu", directory->inode);
	...
	fbr_test_logs("** MKDIR new directory: inode: %lu path: '%s'", file->inode, dirname.name);
